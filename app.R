...
# ── Data ───────────────────────────────────────────────────────────────────────
# NOTE: this is a deployment copy of 03_scripts/shiny_app_ror_growth.R, kept in
# its own folder with only the one data file it needs so that publishing this
# app (shinyapps.io) never uploads the rest of the client project. Data path
# below is a plain relative path (not here::here()) for that reason — re-copy
# this file (and re-run rsconnect::deployApp("07_shiny")) after making edits
# to the main dashboard script.

acs_data   <- readRDS("acs_child_indicators.rds")
...
share_ch   <- setNames(var_meta$col[var_meta$is_share],  var_meta$label[var_meta$is_share])
count_ch   <- setNames(var_meta$col[!var_meta$is_share], var_meta$label[!var_meta$is_share])
all_var_ch <- list("Percentages" = share_ch, "Counts" = count_ch)
